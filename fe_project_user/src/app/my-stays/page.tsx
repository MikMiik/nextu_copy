...
export default function MyStaysPage() {
    ...
    const fetchStays = async () => {
      try {
        ...
        for (const membership of membershipHistory) {
          try {
            const room = await roomService.getRoomInstanceById(
              membership.roomInstanceId
            );
            stayItems.push({ membership, room });
          } catch (roomError) {
            console.error(
              `Failed to fetch room ${membership.roomInstanceId}:`,
              roomError
            );
  ...
  const getStatusBadgeColor = (status: string) => {
    switch (status) {
      ...
      default:
        return "bg-brand-light text-brand-secondary/80";
    }
  };
  ...
  const renderDaysRemaining = (expireAt: string | null) => {
    ...
    if (isBefore(expire, now)) {
      ...
      return (
        <span className="text-brand-secondary/80 font-medium text-xs">
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-brand-light to-brand-light/90">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-brand-primary mx-auto mb-4"></div>
              <p className="text-brand-secondary/80">Loading your stays...</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-center">
              ...
              <p className="text-brand-secondary/80 mb-4">{error}</p>
              <Button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-brand-secondary/80 text-sm">
                Manage your current and past accommodations
              </p>
            </div>
          </div>

          {stays.length === 0 && (
            <div className="bg-brand-light rounded-lg border border-brand-light/30 p-6 text-center">
              <div className="w-12 h-12 bg-brand-light rounded-full flex items-center justify-center mx-auto mb-3">
                <Building className="w-6 h-6 text-brand-secondary/60" />
              </div>
              <h3 className="text-lg font-semibold text-brand-secondary mb-2">
                No stays found
              </h3>
        ...
        {stays.length > 0 && (
          <div className="space-y-3">
            {stays.map((stay) => (
              <Card
                key={stay.membership.requestId}
                className="overflow-hidden hover:shadow-md transition-shadow duration-200"
              >
                <CardContent className="p-0">
                  <div className="flex flex-col md:flex-row">
                    {/* Left: Image - Smaller */}
                    <div className="md:w-56 w-full h-40 md:h-44 bg-brand-light/90 flex-shrink-0">
                      {stay.room.medias && stay.room.medias.length > 0 ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <Building className="w-8 h-8 text-brand-secondary/60" />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex-1 p-4">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="pr-3">
                          ...
                          <p className="text-brand-secondary/80 text-xs">
                            {stay.room.roomCode}
                          </p>
                          <div className="flex items-center gap-1 text-brand-secondary/80 text-xs mt-0.5">
                            <MapPin className="w-3 h-3 text-brand-secondary/60" />
                            <span className="truncate">
                              {stay.room.propertyName}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-3 text-xs">
                        <div className="flex items-center gap-1">
                          <Building className="w-3 h-3 text-brand-secondary/60" />
                          <span className="text-brand-secondary/80">
                            {stay.room.areaInSquareMeters}m²
                          </span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Eye className="w-3 h-3 text-brand-secondary/60" />
                          <span className="text-brand-secondary/80">
                            {stay.room.roomViewName}
                          </span>
                        </div>
                        <div className="flex items-center gap-1">
                          <User className="w-3 h-3 text-brand-secondary/60" />
                          <span className="text-brand-secondary/80">
                            {stay.room.numberOfBeds} beds
                          </span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Package className="w-3 h-3 text-brand-secondary/60" />
                          <span className="text-brand-secondary/80">
                            {stay.room.roomFloorName}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-2">
                        ...
                        <div className="bg-brand-light rounded-md p-2">
                          <div className="flex items-center gap-1 text-xs text-brand-secondary/80">
                            ...
                            <span>
                              Start: {formatDate(stay.membership.startDate)}
                            </span>
                          </div>
                          {stay.membership.expireAt && (
                            <div className="flex items-center gap-1 text-xs text-brand-secondary/80 mt-0.5">
                              <Clock className="w-3 h-3" />
                              <span>
                                Expire: {formatDate(stay.membership.expireAt)}
                              </span>
                            </div>
                          )}
                        </div>

                        <div className="bg-brand-light rounded-md p-2 flex items-center gap-1">
                          <Timer className="w-3 h-3 text-brand-primary" />
                      ...
                      <div className="mt-3 flex gap-2">
                        ...
                        <Button
                          ...
                          className="h-7 text-xs px-3"
                          onClick={() =>
                            handleViewRequest(stay.membership.requestId)
                          }
                        >
