...
export default function PropertyDetail() {
  ...
  const [packageDurationDetails, setPackageDurationDetails] = useState<{
    [key: number]: PackageDurationDetail | null;
  }>({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const propertyRooms = roomsData.filter(
        ...
        const durationPromises = durationIds.map((id) =>
          packageService.getPackageDuration(id)
        );
  ...
  useEffect(() => {
    ...
    if (selectedRoomType !== "all") {
      filtered = filtered.filter(
        (room) => room.roomTypeName === selectedRoomType
      );
    }
    ...
    if (selectedAccommodationOption !== "all") {
      filtered = filtered.filter(
        (room) => room.accommodationOptionId === selectedAccommodationOption
      );
    }
  ...
  const getAccommodationOptionsForRoomType = (roomTypeName: string) => {
    return accommodationOptions.filter(
      (option) => option.roomTypeName === roomTypeName
    );
  };

  // Helper function to get room pricing information
  const getRoomPricingInfo = (room: RoomInstance) => {
    const roomBasicPlans = packageService.getBasicPlansForRoom(
      basicPlans,
      room.accommodationOptionId,
      propertyId
    );
    const roomComboPlans = packageService.getComboPlansForRoom(
      comboPlans,
      roomBasicPlans,
      propertyId
    );
    ...
    // Collect all available packages (filtered basic + combo)
    const allAvailablePackages: Array<{
      type: "basic" | "combo";
      price: number;
      plan: BasicPlan | ComboPlan;
    }> = [];

    // Add filtered basic plans
    filteredBasicPlans.forEach((plan) => {
      allAvailablePackages.push({
        type: "basic",
        price: plan.price,
        plan: plan,
      });
    ...
    // Find the package with lowest price from all available packages
    const lowestPricePackage = allAvailablePackages.reduce((lowest, current) =>
      current.price < lowest.price ? current : lowest
    );

    if (lowestPricePackage.type === "basic") {
      // Basic plan
      const basicPlan = lowestPricePackage.plan as BasicPlan;
      const duration = basicPlan.planDurations?.[0];
      if (!duration) {
        return { hasPackage: false, price: null, duration: null };
      }
      ...
      // Combo plan
      const comboPlan = lowestPricePackage.plan as ComboPlan;
      const duration = comboPlan.packageDurations?.[0];
      if (!duration) {
        return { hasPackage: false, price: null, duration: null };
      }

      // Get duration details for combo
      const durationId = duration.durationId;
      const durationDetail = packageDurationDetails[durationId];
      ...
      return {
        ...
        packageType: "combo",
        duration: {
          value: durationDetail?.value || 1,
          unit: durationDetail?.unit || "Month",
          description: durationDetail?.description || "Flexible duration",
        },
      };
    }
  };
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {roomTypes.map((roomType) => {
              const count = rooms.filter(
                (room) => room.roomTypeName === roomType.name
              ).length;
              return (
                <button
                  key={roomType.id}
        ...
        {selectedRoomType !== "all" && (
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {getAccommodationOptionsForRoomType(selectedRoomType).map(
                (option) => (
                  <button
                    key={option.id}
                  ...
                  >
                    {option.accmodationOptionName} (
                    {
                      rooms.filter(
                        (room) => room.accommodationOptionId === option.id
                      ).length
                    }
                    )
                  </button>
          ...
          {filteredRooms.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-brand-secondary/80">
                Try selecting a different room type or accommodation option.
              </p>
            </div>
          ) : (
                  ...
                  <div className="relative h-48">
                    {room.medias && room.medias.length > 0 ? (
                      ...
                      <div className="w-full h-full bg-brand-light flex items-center justify-center">
                        <Building className="h-12 w-12 text-brand-secondary/60" />
                      </div>
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                    ...
                    <div className="space-y-4">
                          ...
                          {(() => {
                            ...
                            if (pricingInfo.hasPackage) {
                              return (
                                <div className="text-center">
                                  <div className="text-xs text-brand-secondary/80 uppercase tracking-wide mb-1">
                                    Starting Price
                                  </div>
                                  <div className="text-xl font-bold text-brand-primary">
                                    ₫{pricingInfo.price?.toLocaleString()}
                                  </div>
                                  {/* Package type and duration info */}
                                  <div className="flex items-center justify-center gap-2 mt-1">
                                    <Badge
                                      variant="outline"
                                      className={`text-xs px-2 py-0.5 ${
                                        pricingInfo.packageType === "combo"
                                    ...
                                    <span className="text-xs text-brand-secondary/80">
                                      {pricingInfo.duration?.value}{" "}
                                      {pricingInfo.duration?.unit?.toLowerCase()}
                                    </span>
                                  </div>
                                </div>
                              );
                            ...
                            return (
                              <div className="text-center">
                                ...
                                <div className="text-lg text-brand-secondary/80 italic">
                                  Updated later...
                                </div>
                              </div>
                            );
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="flex items-center gap-2">
                          <Maximize className="h-4 w-4 text-brand-secondary/60" />
                          <span>{room.areaInSquareMeters}m²</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Bed className="h-4 w-4 text-brand-secondary/60" />
                          <span>{room.numberOfBeds} beds</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Eye className="h-4 w-4 text-brand-secondary/60" />
                          <span>{room.roomViewName}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Building className="h-4 w-4 text-brand-secondary/60" />
                          <span>{room.roomFloorName}</span>
                        </div>
                      </div>

                      {/* Action button */}
                      <Button className="w-full bg-brand-primary hover:bg-brand-primary/90 text-brand-light">
                        View Details
                      </Button>
                    </div>
                  </CardContent>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-brand-light rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium">
                      {selectedRoom.areaInSquareMeters}m²
                    </span>
                  </div>
                  <div>
              ...
              <div className="bg-brand-light border rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    {/* Main Image */}
                    <div className="relative aspect-[4/3] rounded-lg overflow-hidden border bg-brand-light">
                      {selectedRoom.medias && selectedRoom.medias.length > 0 ? (
                        <>
                          <img
                            src={selectedRoom.medias[currentImageIndex].url}
                            alt={
                              selectedRoom.medias[currentImageIndex]
                          ...
                          {selectedRoom.medias.length > 1 && (
                            <>
                              <button
                                className="absolute left-2 top-1/2 -translate-y-1/2 bg-brand-light/80 hover:bg-brand-light text-brand-secondary p-2 rounded-full shadow-lg transition-all"
                                onClick={() =>
                              ...
                              <button
                                className="absolute right-2 top-1/2 -translate-y-1/2 bg-brand-light/80 hover:bg-brand-light text-brand-secondary p-2 rounded-full shadow-lg transition-all"
                                onClick={() =>
                                  setCurrentImageIndex((prev) =>
                                    prev === selectedRoom.medias.length - 1
                                      ? 0
                                      : prev + 1
                                  )
                                }
                    ...
                    {selectedRoom.medias && selectedRoom.medias.length > 1 && (
                      <div className="grid grid-cols-4 gap-2">
                        {selectedRoom.medias.slice(0, 8).map((media, idx) => (
                          ...
                          >
                            <img
                              src={media.url}
                              alt={
                                media.description || `Room thumbnail ${idx + 1}`
                              }
                              className="w-full h-full object-cover"
                            />
                          </div>
                        ...
                        {selectedRoom.medias.length > 8 && (
                          <div className="aspect-square rounded-lg border-2 border-dashed border-brand-light flex items-center justify-center bg-brand-light">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-white border rounded-lg p-4">
                      ...
                      <div className="mb-4">
                        ...
                        <div className="bg-slate-50 rounded-lg p-3 text-sm text-slate-700">
                          {selectedRoom.descriptionDetails ||
                        ...
                        <div className="space-y-3 text-sm">
                              ...
                              <span
                                ...
                                onClick={() => {
                                  if (selectedRoom.roomTypeName.length > 15) {
                                    alert(
                                      `Full Room Type: ${selectedRoom.roomTypeName}`
                                    );
                                  }
                                }}
                              >
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="font-medium">
                                {selectedRoom.roomSizeName}
                              </span>
                            </div>
                            <div className="flex justify-between items-center">
                              <span className="text-slate-500">Floor:</span>
                              <span className="font-medium">
                                {selectedRoom.roomFloorName}
                              </span>
                            </div>
                          </div>
                    ...
                    <div className="bg-white border rounded-lg p-4">
                      <h5 className="text-sm font-semibold text-slate-800 mb-3">
                        Room-Inclusive Packages
                      </h5>

                      {(() => {
                        const roomBasicPlans =
                          packageService.getBasicPlansForRoom(
                            basicPlans,
                            selectedRoom.accommodationOptionId,
                            propertyId
                          );
                        const roomComboPlans =
                          packageService.getComboPlansForRoom(
                            comboPlans,
                            roomBasicPlans,
                            propertyId
                          );
                        ...
                        // Filter basic plans to only show those with planDurationId = 1 or 2
                        const filteredBasicPlans = roomBasicPlans.filter(
                          (plan) => {
                            if (
                              plan.planDurations &&
                              plan.planDurations.length > 0
                            ) {
                              const duration = plan.planDurations[0];
                              const planDurationId = duration.planDurationId;
                              return (
                                planDurationId === 1 || planDurationId === 2
                              );
                            }
                            return false;
                          }
                          ...
                          return (
                            <div className="text-center py-4">
                              <p className="text-xs text-slate-600 mb-2">
                                This room is not included in any packages at the
                                moment.
                              </p>
                              <Button
                                size="sm"
                              ...
                              <>
                                ...
                                <div className="space-y-2">
                                  {roomComboPlans.map((combo) => {
                                    const duration =
                                      combo.packageDurations?.[0];
                                    const durationId = duration?.durationId;
                                    const durationText = durationId
                                      ? packageDurations[durationId] ||
                                        "Loading..."
                                      : "Flexible duration";
                                    const isOneMonth =
