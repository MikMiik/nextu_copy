...
export default function EventCard({
  ...
  const [ticketAvailability, setTicketAvailability] = useState<
    Record<string, TicketAvailability>
  >({});
  ...
  const fetchTicketAvailability = async (ticketTypeId: string) => {
    ...
    try {
      const response = await api.get(
        `/api/bookings/tickets/${ticketTypeId}/availability`
      );
  ...
  if (variant === "compact") {
    return (
      <Link href={`/ecosystem/events/${event.id}`}>
        <Card
          className="group cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-105 border-0 bg-brand-light h-full flex flex-col"
        ...
        >
          <div className="relative">
            ...
            />
            {event.featured && (
              <Badge className="absolute top-3 left-3 bg-gradient-to-r from-brand-primary to-brand-secondary text-brand-light border-0">
                Featured
              </Badge>
            )}
            <Button
          ...
          <CardContent className="p-4 flex-1">
            ...
            <h3 className="font-semibold text-lg mb-2 line-clamp-2 group-hover:text-brand-primary transition-colors min-h-[3rem]">
              {event.title}
            </h3>

            <p className="text-brand-secondary/80 text-sm mb-3 line-clamp-2 min-h-[3rem]">
              {event.shortDescription}
            </p>

            <div className="space-y-2 text-sm text-brand-secondary/80">
                ...
                <div className="flex items-center gap-2">
                  {(() => {
                    ...
                    return displaySchedule
                      ? `${formatDate(
                          displaySchedule.startTime
                        )} • ${formatScheduleTime(displaySchedule)}`
                      : `${formatDate(event.date)} • ${event.time}`;
                  })()}
                  {getNextUpcomingSchedule() &&
                    getNextUpcomingSchedule() !== event.schedules[0] && (
                      <Badge
                        variant="outline"
                        className="text-xs text-brand-primary border-brand-primary/30"
                      >
                        Next Schedule
                      </Badge>
                    )}
            ...
            <div className="flex items-center justify-between mt-4">
              <div className="flex items-center space-x-1">
                {Object.keys(ticketAvailability).length > 0 ? (
                  ...
                  <span className="text-sm font-medium">
                    {Object.keys(loadingAvailability).some(
                      (key) => loadingAvailability[key]
                    ) ? (
  ...
  return (
    <Link href={`/ecosystem/events/${event.id}`}>
      <Card
        className="group cursor-pointer transition-all duration-300 hover:shadow-xl hover:scale-105 border-0 bg-brand-light h-full flex flex-col"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        <div className="relative">
          ...
          />
          {event.featured && (
            <Badge className="absolute top-4 left-4 bg-gradient-to-r from-brand-primary to-brand-secondary text-brand-light border-0">
              Featured
            </Badge>
          )}
          <Button
        ...
        <CardContent className="p-6 flex-1">
          <div className="flex items-start justify-between mb-3">
            ...
            <Badge variant="outline">{event.level.name}</Badge>
          </div>
          ...
          <div className="space-y-3 text-sm text-gray-500 mb-4">
            ...
            <div className="flex items-center">
              ...
              <span className={getAvailabilityColor()}>
                {Object.keys(ticketAvailability).length > 0 ? (
                  `${calculateAvailableSpots().available}/${
                    calculateAvailableSpots().total
                  } spots available`
                ) : Object.keys(loadingAvailability).some(
                    (key) => loadingAvailability[key]
                  ) ? (
