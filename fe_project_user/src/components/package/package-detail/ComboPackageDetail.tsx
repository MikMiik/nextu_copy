...
import { formatDateWithoutTimezone, calculateEndDate } from "@/utils/dateHelper";
...
export default function ComboPackageDetail({ id, router }: { id: string, router: any }) {
  ...
  const checkAllRoomsAvailability = async (basicId: string, from?: string, to?: string) => {
    ...
    // Chỉ check availability khi user đã đăng nhập
    if (!isLogged()) {
      console.log('[DEBUG] User not logged in, skipping availability check for basicId:', basicId);
      return;
    }
        ...
        try {
          ...
          if (data.available) {
            ...
            results[room.id] = {
              ...
              holdAccountById: data.holdAccountById,
            };
          }
        } catch (error) {
  ...
  const handleDateRangeSelect = async (basicId: string, dates: { moveIn: Date | null; moveOut: Date | null }) => {
    ...
    if (dates.moveIn && dur) {
      moveOut = calculateEndDate(dates.moveIn, dur.planDurationUnit, dur.planDurationValue);
    }
    ...
    if (dates.moveIn && moveOut) {
      localStorage.setItem(`combo_selectedDates_${basicId}`, JSON.stringify({
        moveIn: formatDateWithoutTimezone(dates.moveIn),
        moveOut: formatDateWithoutTimezone(moveOut)
      }));
  ...
  const handlePayNow = async () => {
    ...
    try {
      const res = await api.post("/api/user/memberships/requestMember", {
        packageId: combo.id,
        packageType: "combo",
        selectedStartDate: formatDateWithoutTimezone(accommodationSelectedDates.moveIn),
        requireBooking: true,
                    ...
                    <div className="mb-4">
                      ...
                      <div className="text-slate-600 text-sm leading-relaxed">
                        {(() => {
                          ...
                          const isLong = normalizedDescription.length > 200;
                             ...
                             <div>
                               <div className="mb-3 prose prose-sm max-w-none">
                                 <ReactMarkdown
                                   ...
                                   rehypePlugins={[rehypeSanitize]}
                                 >
                                   {`${normalizedDescription.slice(0, 200)}...`}
                                 </ReactMarkdown>
                               </div>
                               <button
                                 onClick={() => {
                               ...
                               <div id={`desc-${basic.id}`} className="mt-3 hidden prose prose-sm max-w-none">
                                 <ReactMarkdown
                                   remarkPlugins={[remarkGfm]}
                                   rehypePlugins={[rehypeSanitize]}
                                 >
                            ...
                            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="mt-3 text-sm">
                                  {!isUserLoggedIn ? (
                                    <span className="text-blue-600 font-semibold">Login to check availability</span>
                                  ) : (
                                    <>
                                      {isAvailable && <span className="text-green-600 font-semibold">{status}</span>}
                                      {!isAvailable && status && (
                                        <div className="space-y-2">
                                          <span className="text-orange-600 font-semibold block">{status}</span>
                                          {availability.holdAccountById === 1 && (
                                            <button
                                      ...
                                      {!status && accommodationSelectedDates?.moveIn && accommodationSelectedDates?.moveOut && (
                                        <span className="text-slate-500 font-semibold">Checking availability...</span>
                                      )}
                                    </>
                                  )}
                                </div>
                              ...
                              <div className="flex flex-col items-end gap-3">
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <Button
                                    ...
                                    disabled={!isUserLoggedIn ? false : !isAvailable}
                                  >
                                    {!isUserLoggedIn 
                                      ? 'Login to Select'
                                      : isSelected
