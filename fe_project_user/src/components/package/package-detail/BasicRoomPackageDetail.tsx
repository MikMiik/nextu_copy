...
export default function BasicRoomPackageDetail({ id, router }: { id: string, router: any }) {
  ...
  const handleMoveInDateChange = (date: Date | null) => {
    ...
    const moveOut = calculateEndDate(date, duration.planDurationUnit, duration.planDurationValue);
  ...
  const handlePayNow = async () => {
    ...
    try {
      const res = await api.post("/api/user/memberships/requestMember", {
        packageId: pkg.id,
        packageType: "basic",
        selectedStartDate: formatDateWithoutTimezone(selectedDates.moveIn),
        requireBooking: true,
  ...
  const checkAllRoomsAvailability = async (from?: string, to?: string) => {
    ...
    if (!isLogged()) {
      console.log('[DEBUG] User not logged in, skipping availability check');
      return;
    }
  ...
  const handleDateRangeSelect = async (dates: { moveIn: Date | null; moveOut: Date | null }) => {
    ...
    if (dates.moveIn && duration) {
      moveOut = calculateEndDate(dates.moveIn, duration.planDurationUnit, duration.planDurationValue);
    }
                              ...
                              <div className="mt-3 text-sm">
                                {!isUserLoggedIn ? (
                                  ...
                                  <>
                                    ...
                                    {!status && selectedDates.moveIn && selectedDates.moveOut && (
