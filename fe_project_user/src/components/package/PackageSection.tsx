...
export default function PackageSection({
    ...
    async function fetchLocations() {
      try {
        const res = await api.get(
          `/api/user/locations/by-city/${selectedCity}`
        );
        const data = res.data || res;
        setLocations(data);
  ...
  useEffect(() => {
    ...
    async function fetchProperties() {
      try {
        const res = await api.get(
          `/api/user/locations/propertyby-location/${selectedLocation}`
        );
        const data = res.data || res;
        setProperties(data);
  ...
  const [nextServiceNames, setNextServiceNames] = useState<{
    [pkgId: string]: string[];
  }>({});
  ...
  const [accommodationInfo, setAccommodationInfo] = useState<{
    [accommodationId: string]: any;
  }>({});
  const [loadingAccommodation, setLoadingAccommodation] = useState<{
    [accommodationId: string]: boolean;
  }>({});
  const [expandedAccommodations, setExpandedAccommodations] = useState<{
    [key: string]: boolean;
  }>({});
  const [basicPlanDetails, setBasicPlanDetails] = useState<{
    [planId: string]: any;
  }>({});
  const [loadingBasicPlans, setLoadingBasicPlans] = useState<{
    [planId: string]: boolean;
  }>({});
  const [durationDetails, setDurationDetails] = useState<{
    [durationId: string]: any;
  }>({});
  const [loadingDurations, setLoadingDurations] = useState<{
    [durationId: string]: boolean;
  }>({});
  const [fetchedBasicPlans, setFetchedBasicPlans] = useState<Set<string>>(
    new Set()
  );
  const [fetchedDurations, setFetchedDurations] = useState<Set<string>>(
    new Set()
  );
  ...
  const filteredBasic = useMemo(() => {
    ...
    return Array.isArray(basicPlans)
      ? basicPlans.filter((b) => {
          ...
          if (
            b.planDurations &&
            Array.isArray(b.planDurations) &&
            b.planDurations.length > 0
          ) {
    ...
    async function fetchData() {
      ...
      try {
        ...
        basics = await Promise.all(
          basics.map(async (b: any) => {
            try {
              const durationRes = await api.get(
                `/api/membership/BasicPlans/${b.id}/duration`
              );
        ...
        combos = await Promise.all(
          combos.map(async (c: any) => {
            try {
              const durationRes = await api.get(
                `/api/membership/ComboPlans/${c.id}/duration`
              );
  ...
  async function fetchAccommodationInfo(accommodationId: string) {
    ...
    try {
      const res = await api.get(
        `/api/membership/AccommodationOptions/${accommodationId}`
      );
      setAccommodationInfo((prev) => ({
        ...prev,
        [accommodationId]: res.data || res,
      }));
    } catch (err) {
      setAccommodationInfo((prev) => ({ ...prev, [accommodationId]: null }));
    } finally {
      setLoadingAccommodation((prev) => ({
        ...prev,
  ...
  async function fetchDurationDetails(durationId: string) {
    ...
    setFetchedDurations(
      (prev) => new Set(Array.from(prev).concat([durationId]))
    );
    ...
    try {
      ...
      const res = await api.get(
        `/api/membership/PackageDuration/${durationId}`
      );
      setDurationDetails((prev) => ({
        ...prev,
        [durationId]: res.data || res,
      }));
    } catch (err) {
  ...
  const limitedCombos = isPreview
    ? filteredCombo.slice(0, 3)
    : maxPackages
    ? filteredCombo.slice(0, maxPackages)
    : filteredCombo;
  const limitedBasic = isPreview
    ? filteredBasic.slice(0, 3)
    : maxPackages
    ? filteredBasic.slice(0, maxPackages)
    : filteredBasic;
  ...
  if (basicPlans && basicPlans.length > 0) {
    ...
    basicPlans.forEach((plan, index) => {
      ...
      console.log(
        `Plan ${index + 1}: "${
          plan.name
        }" - planDurationId: ${planDurationId}, Should show: ${shouldShow}`
      );
    });
  ...
  useEffect(() => {
    limitedBasic.forEach((pkg) => {
      ...
      if (
        pkg.serviceType === 1 &&
        pkg.entitlements &&
        Array.isArray(pkg.entitlements)
      ) {
        pkg.entitlements.forEach((e: any) => {
          const entitlementId = e.entitlementId;
          if (
            entitlementId &&
            !accommodationInfo[entitlementId] &&
            !loadingAccommodation[entitlementId]
          ) {
            setLoadingAccommodation((prev) => ({
              ...prev,
              [entitlementId]: true,
            ...
            api
              .get(`/api/membership/EntitlementRule/${entitlementId}`)
              .then((res) =>
                setAccommodationInfo((prev) => ({
                  ...prev,
                  [entitlementId]: res.data || res,
                }))
              )
              .catch(() =>
                setAccommodationInfo((prev) => ({
      ...
      limitedCombos.forEach((pkg) => {
        ...
        if (pkg.basicPlanIds && Array.isArray(pkg.basicPlanIds)) {
          pkg.basicPlanIds.forEach((planId: string) => {
            if (
              planId &&
              !fetchedBasicPlans.has(planId) &&
              !loadingBasicPlans[planId] &&
              !planIdsToFetch.includes(planId)
            ) {
        ...
        if (pkg.packageDurations && Array.isArray(pkg.packageDurations)) {
          pkg.packageDurations.forEach((duration: any) => {
            ...
            if (
              durationId &&
              !fetchedDurations.has(durationId) &&
              !loadingDurations[durationId] &&
              !durationIdsToFetch.includes(durationId)
            ) {
      ...
      <div className={`min-h-screen bg-brand-light ${className}`}>
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="text-brand-secondary/80 text-lg">
              Loading packages...
            </p>
          </div>
        </div>
      ...
      {(showLocationFilter || showTypeFilter) && (
        <section className="py-4 px-4 sm:px-6 lg:px-8">
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-4 bg-brand-light/80 backdrop-blur-sm rounded-xl p-4 shadow-sm border border-brand-light/30">
              ...
              {showLocationFilter && (
                <div className="flex flex-col sm:flex-row items-center gap-3 flex-1">
                  ...
                  <div className="relative">
                    <select
                      className="appearance-none bg-brand-light/90 rounded-lg px-4 py-2.5 pr-8 border border-brand-light/30 focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary focus:outline-none text-brand-secondary text-sm font-medium shadow-sm hover:shadow-md transition-all"
                      value={selectedCity}
                      onChange={(e) => {
                    ...
                    <MapPinIcon className="absolute right-2 top-1/2 transform -translate-y-1/2 w-4 h-4 text-brand-secondary/60 pointer-events-none" />
                  </div>

                  <div className="text-brand-secondary/40 hidden sm:block">
                    →
                  </div>
                  ...
                  <div className="relative">
                    <select
                      className="appearance-none bg-brand-light/90 rounded-lg px-4 py-2.5 pr-8 border border-brand-light/30 focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary focus:outline-none text-brand-secondary text-sm font-medium shadow-sm hover:shadow-md transition-all disabled:bg-brand-light/60 disabled:cursor-not-allowed"
                    ...
                    <MapPin className="absolute right-2 top-1/2 transform -translate-y-1/2 w-4 h-4 text-brand-secondary/60 pointer-events-none" />
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <Building2 className="absolute right-2 top-1/2 transform -translate-y-1/2 w-4 h-4 text-brand-secondary/60 pointer-events-none" />
                  </div>
                </div>
              )}
              ...
              {showTypeFilter && (
                <div className="flex-shrink-0">
                  <div className="bg-brand-secondary rounded-lg p-1">
                    <div className="flex">
                      <button
            ...
            {selectedCity && selectedLocation && selectedProperty && (
              <div className="mt-3 text-center">
                <div className="inline-flex items-center gap-2 bg-brand-secondary text-brand-light px-4 py-2 rounded-lg text-sm font-medium">
          ...
          (cities.length > 0 &&
            ...
            <>
              ...
              <div className="text-center mb-6">
                <h2 className="text-xl font-bold text-brand-secondary mb-1">
                  {showType === "basic" ? "Basic Packages" : "Combo Packages"}
                </h2>
                <p className="text-brand-secondary/80 max-w-lg mx-auto text-sm">
                  {showType === "basic"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-center">
                  {limitedBasic.length === 0 ? (
                    <div className="col-span-full text-center py-12">
                      <div className="bg-brand-light/80 rounded-xl p-6 shadow-sm">
                        <Building2 className="w-12 h-12 text-brand-secondary/60 mx-auto mb-3" />
                        <h3 className="text-lg font-semibold text-brand-secondary/80 mb-2">
                          No Basic Packages Available
                        </h3>
                        <p className="text-brand-secondary/60 text-sm">
                          No basic packages are currently available for the
                          selected property.
                        </p>
                      </div>
                    </div>
                    ...
                    limitedBasic.map((pkg, idx) => (
                      <Card
                        key={pkg.id}
                        className={`w-full relative overflow-hidden rounded-2xl border-0 shadow-lg hover:shadow-xl transition-all duration-500 transform hover:-translate-y-2 min-h-[480px] ${
                          idx === 0
                            ? "ring-2 ring-brand-primary border-brand-primary shadow-2xl"
                            : ""
                        }`}
                      >
                        {/* Popular Badge */}
                        {idx === 0 && (
                          <div className="absolute top-4 right-4 bg-gradient-to-r from-brand-primary to-brand-secondary text-brand-light px-3 py-1.5 rounded-full text-xs font-semibold z-10 shadow-lg">
                        ...
                        <div className="relative h-48 bg-brand-light/30 overflow-hidden">
                          {/* Background Pattern */}
                          <div className="absolute inset-0 opacity-10">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-brand-primary rounded-full -translate-y-16 translate-x-16"></div>
                            <div className="absolute bottom-0 left-0 w-24 h-24 bg-brand-secondary/30 rounded-full translate-y-12 -translate-x-12"></div>
                          </div>

                          {/* Content */}
                          <div className="relative z-10 p-6 text-center h-full flex flex-col justify-center">
                            <div className="mb-3">
                              <Badge className="bg-brand-light/80 text-brand-secondary border-0 px-3 py-1 text-xs font-medium">
                                {pkg.nextUServiceName || "Basic Package"}
                              </Badge>
                            </div>
                            <CardTitle className="text-xl font-bold text-brand-secondary mb-3 leading-tight">
                              {pkg.name}
                            </CardTitle>
                            <div className="flex items-center justify-center mb-2">
                              ...
                              <span className="text-lg text-brand-secondary/80 ml-1">
                                $
                              </span>
                            </div>
                            {pkg.planDurations &&
                              pkg.planDurations.length > 0 && (
                                <div className="text-brand-secondary/80 text-sm font-medium">
                                  {pkg.planDurations[0]
                                    .planDurationDescription ||
                                    `${pkg.planDurations[0].planDurationValue} ${pkg.planDurations[0].planDurationUnit}`}
                                </div>
                              )}
                          </div>
                        </div>
                        ...
                        <CardContent className="p-5 space-y-4">
                          ...
                          <div className="bg-brand-light/10 rounded-lg p-4">
                            <h4 className="font-semibold text-brand-secondary mb-3 flex items-center text-sm">
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-4">
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                <span className="text-sm font-medium text-brand-secondary font-mono">
                                  {pkg.code}
                                </span>
                              </div>
                            </div>
                            ...
                            {/* Divider */}
                            <div className="border-t border-brand-light/30 my-3"></div>

                            {/* Description */}
                            <div className="h-[4rem]">
                              <div className="text-xs text-brand-secondary/60 mb-2">
                                Description
                              </div>
                              <div className="h-[3rem] overflow-hidden">
                                <p className="text-brand-secondary/80 text-sm leading-relaxed line-clamp-3">
                                  {pkg.description ||
                          ...
                          {/* Services Section */}
                          <div>
                            <h4 className="font-semibold text-brand-secondary mb-2 flex items-center text-sm">
                              <Zap className="h-4 w-4 mr-2 text-brand-primary" />
                              Included Services
                            </h4>
                            {pkg.acomodations && pkg.acomodations.length > 0 ? (
                              <div className="space-y-2">
                                {pkg.acomodations.map((a: any, i: number) => {
                                  ...
                                  const loading =
                                    loadingAccommodation[accommodationId];
                                  const expanded =
                                    expandedAccommodations[accommodationId];
                                  const hasDetail =
                                    info &&
                                    (info.capacity !== undefined ||
                                      info.pricePerNight !== undefined ||
                                      info.description);
                                  return (
                                    <div
                                      key={accommodationId || i}
                                      className="bg-brand-light/80 rounded-lg p-3 border border-brand-light/30"
                                    >
                                      <div className="flex items-center justify-between">
                                        <div className="flex items-center">
                                          {loading ? (
                                            <>
                                              <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-brand-primary mr-2"></div>
                                            ...
                                            <span className="text-brand-secondary/60 italic text-sm">
                                              No info
                                            </span>
                                          )}
                                        </div>
                                        {hasDetail && (
                                          <button
                                            className={`p-1 rounded-full hover:bg-brand-light/60 transition-all duration-200 ${
                                              expanded
                                                ? "bg-brand-light/60"
                                                : ""
                                            }`}
                                            onClick={() =>
                                              setExpandedAccommodations(
                                                (prev) => ({
                                                  ...prev,
                                                  [accommodationId]:
                                          ...
                                          >
                                            <span
                                              className={`transition-transform duration-200 text-brand-secondary/60 ${
                                                expanded ? "rotate-180" : ""
                                              }`}
                                            >
                                              ▼
                                      ...
                                      {expanded && info && (
                                        <div className="mt-3 pt-3 border-t border-brand-light/30 space-y-2">
                                          {typeof info.capacity !==
                                            "undefined" && (
                                          ...
                                          {typeof info.pricePerNight !==
                                            ...
                                            <div className="flex justify-between text-xs">
                                              ...
                                              <span className="font-medium text-brand-secondary">
                                                {info.pricePerNight.toLocaleString()}
                                                ₫
                                              </span>
                                            </div>
                                          )}
                              ...
                              <div className="space-y-2">
                                {pkg.entitlements.map((e: any, i: number) => {
                                  ...
                                  const loading =
                                    loadingAccommodation[entitlementId];
                                  const expanded =
                                    expandedAccommodations[entitlementId];
                                  return (
                                    <div
                                      key={entitlementId || i}
                                      className="bg-brand-light/80 rounded-lg p-3 border border-brand-light/30"
                                    >
                                        ...
                                        <div className="flex items-center">
                                          {loading ? (
                                            ...
                                            <>
                                              <Check className="h-4 w-4 text-green-500 mr-2 flex-shrink-0" />
                                              <span className="text-brand-secondary font-medium text-sm">
                                                {info && info.nextUServiceName
                                                  ? info.nextUServiceName
                                                  : e.nextUSerName ||
                                                    e.nextUServiceName}
                                              </span>
                                            </>
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            ...
                                            if (!info && !loading) {
                                              ...
                                              api
                                                .get(
                                                  `/api/membership/EntitlementRule/${entitlementId}`
                                                )
                                                .then((res) =>
                                      ...
                                      {expanded &&
                                        (loading ? (
                                          <div className="mt-3 pt-3 border-t border-brand-light/30 text-xs text-brand-secondary/60">
                                            Loading...
                                          </div>
                                        ) : (
                                          info && (
                              ...
                              <div className="bg-brand-light/10 rounded-lg p-4 text-center">
                          ...
                          {/* CTA Button */}
                          <Button
                            className="w-full rounded-xl bg-gradient-to-r from-brand-primary to-brand-secondary hover:from-brand-secondary hover:to-brand-primary text-brand-light font-semibold py-3 shadow-lg hover:shadow-xl transition-all duration-300 text-sm"
                            onClick={() =>
                              handleRequestPackage(pkg.id, pkg.id, "basic")
                            }
                  ...
                  {isPreview && limitedBasic.length > 0 && (
                    <div className="col-span-full flex justify-center mt-6">
                      <Link href="/ecosystem/Co-living">
                        <Button className="rounded-lg px-6 py-2 bg-brand-secondary hover:bg-brand-primary text-brand-light hover:shadow-md transition-all duration-200 text-sm">
                          View all packages
                        </Button>
                      </Link>
                    </div>
              ...
              {showType === "combo" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-center">
                  {limitedCombos.length === 0 ? (
                    <div className="col-span-full text-center py-12">
                      <div className="bg-brand-light/80 rounded-xl p-6 shadow-sm">
                        <Sparkles className="w-12 h-12 text-brand-secondary/60 mx-auto mb-3" />
                        <h3 className="text-lg font-semibold text-brand-secondary/80 mb-2">
                          No Combo Packages Available
                        </h3>
                        <p className="text-brand-secondary/60 text-sm">
                          No combo packages are currently available for the
                          selected location.
                        </p>
                      </div>
                    </div>
                    ...
                    limitedCombos.map((pkg, idx) => (
                      <Card
                        ...
                        className={`w-full relative overflow-hidden rounded-2xl border-0 shadow-lg hover:shadow-xl transition-all duration-500 transform hover:-translate-y-2 min-h-[480px] ${
                          idx === 0
                            ? "ring-2 ring-brand-secondary border-brand-secondary shadow-2xl"
                            : ""
                        }`}
                      >
                        {/* Popular Badge */}
                        {idx === 0 && (
                          <div className="absolute top-4 right-4 bg-gradient-to-r from-brand-secondary to-brand-primary text-brand-light px-3 py-1.5 rounded-full text-xs font-semibold z-10 shadow-lg">
                            <div className="flex items-center">
                              <Star className="h-3 w-3 mr-1.5" />
                              <span>Most Popular</span>
                            </div>
                        ...
                        <div className="relative h-48 bg-brand-light/30 overflow-hidden">
                          ...
                          <div className="absolute inset-0 opacity-10">
                            ...
                            <div className="absolute bottom-0 left-0 w-24 h-24 bg-brand-primary rounded-full translate-y-12 -translate-x-12"></div>
                          </div>
                          ...
                          <div className="relative z-10 p-6 text-center h-full flex flex-col justify-center">
                            ...
                            <div className="text-brand-secondary/80 text-sm font-medium mb-2">
                              <span className="bg-brand-secondary/10 text-brand-secondary px-2 py-1 rounded-full text-xs font-semibold">
                                {pkg.discountRate * 100}% discount
                              </span>
                            </div>
                            {/* Duration Display */}
                            {pkg.packageDurations &&
                              pkg.packageDurations.length > 0 && (
                                <div className="text-brand-secondary/80 text-sm font-medium">
                                  {(() => {
                                    const duration = pkg.packageDurations[0];
                                    const durationId = duration.durationId;
                                    const durationInfo =
                                      durationDetails[durationId];
                                    const loading =
                                      loadingDurations[durationId];
                        ...
                        <CardContent className="p-5 space-y-4">
                          ...
                          <div>
                            <h4 className="font-semibold text-brand-secondary mb-2 flex items-center text-sm">
                              <Sparkles className="h-4 w-4 mr-2 text-brand-primary" />
                              Included Packages
                            </h4>
                              ...
                              <div className="space-y-2">
                                ...
                                {(() => {
                                  ...
                                  pkg.basicPlanIds.forEach((planId: string) => {
                                    ...
                                    if (planDetail) {
                                      const type =
                                        planDetail.nextUServiceName || "Other";
                                      if (!plansByType[type]) {
                                        plansByType[type] = [];
                                      }
                                  ...
                                  return Object.entries(plansByType).map(
                                    ([type, plans]) => (
                                      <div
                                        key={type}
                                        className="bg-brand-light/80 rounded-lg p-3 border border-brand-light/30"
                                      >
                                        ...
                                        <div className="ml-6 space-y-1">
                                          {plans.map((plan, planIdx) => (
                                            ...
                                            >
                                              <div className="w-2 h-2 bg-brand-light/30 rounded-full mr-2 flex-shrink-0"></div>
                                              <span className="text-brand-secondary/80 text-xs">
                                                {plan.name}
                                              </span>
                                            </div>
                                          ))}
                                ...
                                {pkg.basicPlanIds.some(
                                  (planId: string) => loadingBasicPlans[planId]
                                ) && (
                                  <div className="bg-brand-light/80 rounded-lg p-3 border border-brand-light/30">
                                    <div className="flex items-center">
                                      <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-brand-primary mr-2"></div>
                                      <span className="text-brand-secondary/60 text-sm">
                          ...
                          {/* CTA Button */}
                          <Button
                            className="w-full rounded-xl bg-gradient-to-r from-brand-secondary to-brand-primary hover:from-brand-primary hover:to-brand-secondary text-brand-light font-semibold py-3 shadow-lg hover:shadow-xl transition-all duration-300 text-sm"
                            onClick={() =>
                              handleRequestPackage(pkg.id, pkg.id, "combo")
                            }
                            disabled={requestingPackageId === pkg.id}
                          >
                            {requestingPackageId === pkg.id ? (
            ...
            <div className="text-center py-12">
              <div className="bg-brand-light/80 rounded-xl p-8 shadow-sm max-w-xl mx-auto">
                <Building2 className="w-16 h-16 text-brand-secondary/60 mx-auto mb-4" />
                <h3 className="text-lg font-semibold text-brand-secondary/80 mb-3">
                  No Location Selected
                </h3>
                <p className="text-brand-secondary/60 mb-4 text-sm">
                  Please select a city, location, and property above to view
                  available packages.
                </p>
